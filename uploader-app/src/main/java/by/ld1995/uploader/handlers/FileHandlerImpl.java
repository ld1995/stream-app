...
public class FileHandlerImpl implements FileHandler {
    ...
    private final static Function<String, String> GET_LANGUAGE = content -> content.substring(content.indexOf(LANGUAGE) + LANGUAGE.length(), content.indexOf("\n\n")).trim();

    private final static Function<String, String[]> GET_FILENAME_AND_EXTENSION = filename -> filename.split(REGEX_FOR_SPLITTING_FILENAME);

    private final static BiFunction<String, List<String>, Mono<Boolean>> EXTENSION_VALIDATION = (filename, extension) -> Mono.just(
    ...
    private final static BiFunction<ServerRequest, List<String>, Flux<FilePart>> GET_FILE_PART_FORM_REQUEST = (req, extensions) ->
            req.multipartData()
    ...
    private final static Function<FilePart, Flux<String>> GET_CONTENT_OF_FILE = filePart -> filePart.content()
    ...
    private final static Function<String, Subtitle> BUILD_SUBTITLE = content -> {
    ...
    private final static Function<FilePart, VideoInfo> BUILD_VIDEO_INFO = filePart -> {
    ...
    private final static BiFunction<VideoInfo, List<Subtitle>, VideoInfo> ADD_SUBTITLES = (videoInfo, subtitles) -> {
    ...
    private final static Mono<ServerResponse> BUILD_SUCCESS_SERVER_RESPONSE = ServerResponse.ok().body(Mono.just(SUCCESS_RESPONSE), String.class);
    ...
    private final Consumer<FilePart> saveVideoToDisk = filePart ->
    ...
    public Mono<ServerResponse> uploadSubtitles(final ServerRequest request) {
        Optional<String> videoId = request.queryParam("videoId");
        if (videoId.isPresent()) {
            Consumer<List<Subtitle>> saveSubtitleToDB = newSubtitles -> videoInfoRepository.findById(videoId.get())
            ...
            return ServerResponse.badRequest().body(Mono.just("Required request parameter 'videoId' is empty!"), String.class);
        }
    }
